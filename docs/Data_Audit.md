...
| **Notable Segments** (Velocity Under Load) | `sales_cycle_days` by `sales_agent` | **Cycle Length Signal.** Darcel's cycle holds below the Central team average (45.84 vs. 48.49 days) despite carrying 2.5× the deal load, confirming the concentration risk is structural, not yet reflected in individual performance degradation. No single rep can actively manage 194 deals regardless of throughput rate. | Sales Ops / RevOps | Provides a secondary strain indicator beyond active deal count. If cycle length is lengthening under pipeline load, it validates that overconcentration is beginning to affect closing throughput — not just pipeline size. |